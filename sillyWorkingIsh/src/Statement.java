...
public abstract class Statement {
	...
	public static Statement getStatement(TokenStream input) throws Exception {
		...
		} else if (first.toString().equals("const")) {
			return new ConstantDeclaration(input);
		} else if (first.toString().equals("local")) {
			return new Local(input);
		} else if (first.toString().equals("sub")) {
			return new subDec1(input);
